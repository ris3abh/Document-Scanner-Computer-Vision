...
plotTheStatistics(grayImage, 4, image)

function plotTheStatistics(grayImage, sigma, image)
    k = 2 * ceil(3 * sigma) + 1; 
    [smoothImage, ~] = edgeDetectionOfImage(grayImage, k, sigma);
    
    edges = findOptimumEdges(smoothImage);
    detectedEdges = removeBackgroundNoise(edges);
    smoothImage = detectedEdges;

    [houghTransformLine, rRange, thetaRange] = houghTransformForLine(detectedEdges);
    ...
    rectifiedImage = rectifyImage(image, cornerPoints);
    figure;
    imshow(rectifiedImage);

end

function edges = findOptimumEdges(image)
    bestScore = 0;
    bestEdges = [];
    for k = 3:2:30
        for sigma = 0.5:0.1:3.0
            [~, edgePixels] = edgeDetectionOfImage(image, k, sigma);
...
function [smoothImage, edgePixels] = edgeDetectionOfImage(image, k, sigma)
    kernel = zeros(k, k);
    center = (k + 1) / 2;
    for i = 1:k
        for j = 1:k
...
function detectedEdges = removeBackgroundNoise(edgePixels)
    ...
    detectedEdges = (labeledImage == maxLabel);
end

function labeledImage = depthFirstSearch(binarizedImage, labeledImage, label, i, j)
    [height, width] = size(binarizedImage);
    stackSize = height * width;
    stack = zeros(stackSize, 2); 

    stackIndex = 1; 
    stack(stackIndex, :) = [i, j];  

    while stackIndex > 0
        currPos = stack(stackIndex, :);
        ...
        if currPos(1) >= 1 && currPos(1) <= height && currPos(2) >= 1 && currPos(2) <= width
            if binarizedImage(currPos(1), currPos(2)) == 1 && labeledImage(currPos(1), currPos(2)) == 0
                ...
                stackIndex = stackIndex + 1;
                stack(stackIndex, :) = [currPos(1)-1, currPos(2)];  
                stackIndex = stackIndex + 1;
                stack(stackIndex, :) = [currPos(1)+1, currPos(2)];
                stackIndex = stackIndex + 1;
                stack(stackIndex, :) = [currPos(1), currPos(2)-1];
                stackIndex = stackIndex + 1;
                stack(stackIndex, :) = [currPos(1), currPos(2)+1];
            end
        end
...
function [houghTransformLine, rRange, thetaRange] = houghTransformForLine(detectedEdges)
    thetaRange = 0:360;
    w = size(detectedEdges,2);
    h = size(detectedEdges,1);
    rMax = round(sqrt(w^2 + h^2));
    rRange = -rMax:rMax;
    
    houghTransformLine = zeros(length(rRange), length(thetaRange));
    
    [y,x] = find(detectedEdges);
    edges = length(x);
    for i = 1:edges
...
function localMaxima = findLocalMaxima(houghTransformLine)
    [rows, cols] = size(houghTransformLine);
    localMaxima = false(size(houghTransformLine));

    for i = 2:rows-1
        for j = 2:cols-1
            pixelValue = houghTransformLine(i, j);

            % Check if the pixel is greater than its neighbors
            if pixelValue > houghTransformLine(i-1, j-1) && ...
               pixelValue > houghTransformLine(i-1, j) && ...
               pixelValue > houghTransformLine(i-1, j+1) && ...
               pixelValue > houghTransformLine(i, j-1) && ...
               pixelValue > houghTransformLine(i, j+1) && ...
               pixelValue > houghTransformLine(i+1, j-1) && ...
               pixelValue > houghTransformLine(i+1, j) && ...
               pixelValue > houghTransformLine(i+1, j+1)
